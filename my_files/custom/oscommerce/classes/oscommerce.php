...
// +-----------------------------------------------------------------+
//  Path: /my_files/custom/oscommerce/classes/oscommerce.php
//

class oscommerce extends parser {
  var $arrOutput = array();
  var $resParser;
  var $strXML;
  ...
  function submitXML($id, $action = '', $hide_success = false, $inc_image = true) {
	...
	switch ($action) {
	  ...
	  case 'confirm':
		...
		break;
	  default:
		$messageStack->add(OSCOMMERCE_INVALID_ACTION, 'error');
		return false;
	}
//echo 'Submit XML string = <pre>' . $this->strXML . '</pre><br>';
	$this->response = $this->doCURLRequest('POST', OSCOMMERCE_URL . '/soap/' . $url, $this->strXML);
//echo 'XML response (at the PhreeBooks side from osCommerce) => <pre>' . htmlspecialchars($this->response) . '</pre><br>' . chr(10);
...
//echo 'Parsed string = '; print_r($this->arrOutput); echo '<br>';
	...
	if ($this->code == 0) {
	  ...
	  $messageStack->add(OSCOMMERCE_TEXT_ERROR . $this->code . ' - ' . $this->text, strtolower($this->result));
	  return false;
	}
  }
...
/*************************************************************************************/
  function buildProductUploadXML($id, $inc_image = true) {
	...
	if ($result->RecordCount() <> 1) {
	  $messageStack->add(OSCOMMERCE_INVALID_SKU,'error');
	  return false;
	}
	$this->sku = $result->fields['sku'];
	if (OSCOMMERCE_USE_PRICE_SHEETS == '1') {
	  $sql = "select id, default_levels from " . TABLE_PRICE_SHEETS . " 
		where '" . date('Y-m-d',time()) . "' >= effective_date 
		and sheet_name = '" . OSCOMMERCE_PRICE_SHEET . "' and inactive = '0'";
	  $default_levels = $db->Execute($sql);
	  if ($default_levels->RecordCount() == 0) {
		$messageStack->add(OSCOMMERCE_ERROR_NO_PRICE_SHEET . OSCOMMERCE_PRICE_SHEET, 'error');
		return false;
	  }
	...
	if ($inc_image && $result->fields['image_with_path']) { // image file
	  ...
	  if (file_exists($filename)) {
		if ($handle = fopen($filename, "rb")) {
		  ...
		  fclose($handle);
		  // osCommerce only support one level, so we'll use the first path dir and filename only 
	...
	$this->strXML .= '<AccessUserID>' . OSCOMMERCE_USERNAME . '</AccessUserID>' . chr(10);
	$this->strXML .= '<AccessPassword>' . OSCOMMERCE_PASSWORD . '</AccessPassword>' . chr(10);
...
//	$this->strXML .= '<StoreID>' . OSCOMMERCE_STORE_ID . '</StoreID>' . chr(10);
//	$this->strXML .= '<SalesRepID>' . OSCOMMERCE_SALES_REP_ID . '</SalesRepID>' . chr(10);
...
// TBD need mappping for TaxClassType
	...
	$this->strXML .= '<TaxClassType>' . OSCOMMERCE_PRODUCT_TAX_CLASS . '</TaxClassType>' . chr(10);
	// Price Levels
	...
	if (OSCOMMERCE_USE_PRICE_SHEETS) {
...
if (file_exists(DIR_FS_MY_FILES . 'custom/oscommerce/extra_actions/extra_product_attrs.php')) {
       include (DIR_FS_MY_FILES . 'custom/oscommerce/extra_actions/extra_product_attrs.php');
}
...
/*************************************************************************************/
  function buildProductSyncXML() { 
	...
	if ($result->RecordCount() == 0) {
	  $messageStack->add(OSCOMMERCE_ERROR_NO_ITEMS, 'error');
	  return false;
	}
...
/*************************************************************************************/
  function buildConfirmXML() {
	...
	if ($result->RecordCount() == 0) {
	  $messageStack->add(OSCOMMERCE_ERROR_CONFRIM_NO_DATA, 'caution');
	  return false;
	}
	// foreach shipment, fetch the PO Number (it is the osCommerce order number)
	  ...
	  $details = $db->Execute("select so_po_ref_id from " . TABLE_JOURNAL_MAIN . " 
		...
		if ($details->fields['so_po_ref_id']) {
		  ...
		  if ($details->RecordCount() == 1) {
		    $message = sprintf(OSCOMMERCE_CONFIRM_MESSAGE, $this->post_date, $methods[$result->fields['carrier']]['title'], $methods[$result->fields['carrier']][$result->fields['method']], $result->fields['tracking_id']);
		    $this->strXML .= '<Order>'   . chr(10);
		    $this->strXML .= '<ID>'      . $details->fields['purchase_invoice_id'] . '</ID>' . chr(10);
		    $this->strXML .= '<Status>'  . ($details->fields['closed'] ? OSCOMMERCE_STATUS_CONFIRM_ID : OSCOMMERCE_STATUS_PARTIAL_ID) . '</Status>' . chr(10);			
