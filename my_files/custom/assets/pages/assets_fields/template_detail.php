...
// Build the page
?>
<div class="pageHeading"><?php echo ASSETS_FIELD_HEADING_TITLE; ?></div>
...
<?php
	echo html_radio_field('entry_type', $value = 'text', ($cInfo->entry_type=='text' ? true : false), '', ($system_disable ? ' disabled ' : ''));
	echo '&nbsp;' . INV_LABEL_TEXT_FIELD;
	echo '<br>' . html_radio_field('entry_type', $value = 'html', ($cInfo->entry_type=='html' ? true : false), '', ($system_disable ? ' disabled ' : ''));
...
<?php
	...
	echo '<br>' . html_input_field('text_length', ($cInfo->text_length ? $cInfo->text_length : '32'), ($system_disable ? ' disabled ' : '') . 'size="10" maxlength="9"', false);
	echo '<br>' . INV_LABEL_DEFAULT_TEXT_VALUE . '<br>' . INV_LABEL_MAX_255;
	echo '<br>' . html_textarea_field('text_default', true, 35, 6, $cInfo->text_default, ($system_disable ? ' disabled ' : ''));
?>
		</td>
	  </tr>
	  <tr>
		<td class="main" valign="top">
<?php
	echo html_radio_field('entry_type', $value = 'hyperlink', ($cInfo->entry_type=='hyperlink' ? true : false), '', ($system_disable ? ' disabled ' : ''));
	echo '&nbsp;' . INV_LABEL_HYPERLINK;
	echo '<br>' . html_radio_field('entry_type', $value = 'image_link', ($cInfo->entry_type=='image_link' ? true : false), '', ($system_disable ? ' disabled ' : ''));
	echo '&nbsp;' . INV_LABEL_IMAGE_LINK;
	echo '<br>' . html_radio_field('entry_type', $value = 'inventory_link', ($cInfo->entry_type=='inventory_link' ? true : false), '', ($system_disable ? ' disabled ' : ''));
...
<?php
	...
	echo '<br>' . html_textarea_field('link_default', true, 35, 3, $cInfo->link_default, ($system_disable ? ' disabled ' : ''));
?>
		</td>
	  </tr>
	  <tr>
		<td class="main" valign="top">
<?php
	echo html_radio_field('entry_type', $value = 'integer', ($cInfo->entry_type=='integer' ? true : false), '', ($system_disable ? ' disabled ' : ''));
...
<?php
	...
	echo '<br>' . html_pull_down_menu('integer_range', $integer_pulldown_array, $cInfo->integer_range, ($system_disable ? ' disabled ' : ''));
	echo '<br>' . INV_LABEL_DEFAULT_TEXT_VALUE;
	echo html_input_field('integer_default', $cInfo->integer_default, ($system_disable ? ' disabled ' : '') . 'size="16"', false);
?>
		</td>
	  </tr>
	  <tr>
