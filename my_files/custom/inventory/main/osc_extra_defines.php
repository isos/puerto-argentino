...
// +-----------------------------------------------------------------+
//  Path: /my_files/custom/inventory/main/extra_defines.php
//
...
@include_once(DIR_FS_MY_FILES . 'custom/oscommerce/config.php'); // pull the current osCommerce config info, if it is there
@include_once(DIR_FS_MY_FILES . 'custom/oscommerce/language/' . $_SESSION['language'] . '/language.php');
function add_extra_action_bar_buttons($query_fields) {
  $output = '';
  if (defined('OSCOMMERCE_URL') && $_SESSION['admin_security'][SECURITY_ID_MAINTAIN_INVENTORY] > 1 && $query_fields['catalog'] == '1') {
    $output .= html_icon('../../../../my_files/custom/oscommerce/images/oscommerce.gif', OSCOMMERCE_IVENTORY_UPLOAD, 'small', 'onclick="submitSeq(' . $query_fields['id'] . ', \'osc_upload\')"', '16', '16') . chr(10);
  }
  return $output;
}
...
if (defined('OSCOMMERCE_URL')) $extra_fields[] = 'catalog';
