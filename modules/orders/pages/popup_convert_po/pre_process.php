...
// +-----------------------------------------------------------------+
//  Path: /modules/orders/pages/popup_convert_po/pre_process.php
//
  ...
  case 'save':
	...
	for ($i = 0; $i < sizeof($temp_rows); $i++) {
	  ...
	  if ($temp_rows[$i]['sku']) {
		...
		if ($result->fields['vendor_id']) $vendor_id = $result->fields['vendor_id']; // save preferred vendor (takes last one)
		$order->journal_rows[] = array(
		  'gl_type'      => GL_TYPE,
		  'sku'          => $temp_rows[$i]['sku'],
		  'qty'          => $temp_rows[$i]['qty'],
		  'description'  => $result->fields['description_purchase'],
		  'debit_amount' => $result->fields['item_cost'],
	...
	if (!$vendor_id) {
	  $messageStack->add('No valid vendor could be found, make sure at least on sku has a prefered vendor setup for the inventory item.','error');
      break;
	}
