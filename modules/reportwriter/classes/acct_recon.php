...
// +-----------------------------------------------------------------+
//  Path: /modules/reportwriter/classes/acct_recon.php
//
...
if (file_exists(DIR_FS_WORKING . 'language/' . $_SESSION['language'] . '/classes/acct_recon.php')) {
  require_once(DIR_FS_WORKING . 'language/' . $_SESSION['language'] . '/classes/acct_recon.php');
} else {
  require_once(DIR_FS_WORKING . 'language/en_us/classes/acct_recon.php');
}
...
class acct_recon {
	...
	function load_report_data($Prefs, $Seq) {
	    ...
	    $gl_account        = $Prefs['fromvalue1']; // assumes that the gl account is the first criteria
		...
		$sql = "SELECT m.id, m.post_date, i.debit_amount, i.credit_amount, m.purchase_invoice_id, i.description " .
				"FROM " . TABLE_JOURNAL_MAIN . " m " .
						"INNER JOIN " . TABLE_JOURNAL_ITEM . " i " .
								"ON m.id = i.ref_id " .
