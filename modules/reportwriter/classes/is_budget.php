...
// +-----------------------------------------------------------------+
//  Path: /modules/reportwriter/classes/is_budget.php
//
...
if (file_exists(DIR_FS_WORKING . 'language/' . $_SESSION['language'] . '/classes/is_budget.php')) {
  require(DIR_FS_WORKING . 'language/' . $_SESSION['language'] . '/classes/is_budget.php');
} else {
  require(DIR_FS_WORKING . 'language/en_us/classes/is_budget.php');
}
require_once(DIR_FS_MODULES . 'gen_ledger/functions/gen_ledger.php'); // required for chart of account type mapping
...
class is_budget {
  ...
  function load_report_data($Prefs, $Seq) {
	global $db;
	$this->period = substr($Prefs['datedefault'], 2);
	...
	foreach ($Prefs['FieldListings'] as $value) {
	  $line[] = ProcessData($temp[$value['fieldname']], $value['params']['processing']);
	}
	...
	$line = array(0 => 'd');
	foreach ($Prefs['FieldListings'] as $value) {
	  $line[] = ProcessData($grand_total[$value['fieldname']], $value['params']['processing']);
	}
	$this->inc_stmt_data[] = $line;
	// Less Expenses
	$this->add_heading_line();
...
//		$beg_balance->MoveNext();
  ...
  function add_heading_line($title = '') {
	global $Prefs;
	$line = array('d', $title);
	for ($i = 0; $i < sizeof($Prefs['FieldListings']) - 1; $i++) $line[] = '';
	$this->inc_stmt_data[] = $line;
  }
