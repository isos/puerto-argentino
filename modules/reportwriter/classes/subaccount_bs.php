...
// +-----------------------------------------------------------------+
//  Path: /modules/reportwriter/classes/subaccount_bs.php
//
...
if (file_exists(DIR_FS_WORKING . 'language/' . $_SESSION['language'] . '/classes/subaccount_bs.php')) {
  require(DIR_FS_WORKING . 'language/' . $_SESSION['language'] . '/classes/subaccount_bs.php');
} else {
  require(DIR_FS_WORKING . 'language/en_us/classes/subaccount_bs.php');
}
...
class subaccount_bs {
  ...
  function load_report_data($Prefs, $Seq) {
	...
	$this->max_num_levels = sizeof($Prefs['FieldListings']) - 1; // maximum number of indents
