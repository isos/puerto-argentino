...
// +-----------------------------------------------------------------+
//  Path: /modules/setup/classes/inv_tabs.php
//

class inv_tabs {

  function inv_tabs() {
  	$this->security_id = $_SESSION['admin_security'][SECURITY_ID_INV_TABS];
	$this->db_table = TABLE_INVENTORY_CATEGORIES;
	$this->title = INV_CAT_HEADING_TITLE;
...
//	$toolbar->icon_list['update']['text'] = 'Text override here';
  ...
  function build_main_html() {
	...
	$heading_array = array(
		'category_name' => INV_HEADING_CATEGORY_NAME,
    ...
    while (!$result->EOF) {
	  ...
	  if ($this->security_id > 3) $output .= html_icon('emblems/emblem-unreadable.png', TEXT_DELETE, 'small', 'onclick="if (confirm(\'' . INV_INFO_DELETE_INTRO . '\')) submitSeq(\'' . $result->fields['category_id'] . '\', \'delete\')"') . chr(10);
      $output .= '    </td>' . chr(10);
      $output .= '  </tr>' . chr(10);
      $result->MoveNext();
    }
