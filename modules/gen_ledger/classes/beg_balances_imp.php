...
// +-----------------------------------------------------------------+
//  Path: /modules/gen_ledger/classes/beg_balances_imp.php
//
...
//echo 'parsed string = '; print_r($this->records); echo '<br />';
		...
		while ($row_id < count($this->records)) {
			...
			$this->records[$row_id]['post_date'] = gen_db_date_short($current_order['post_date']); // from mm/dd/yyyy to YYYY-MM-DD
	...
	function cyberParse($upload_name) {
		...
		$titles = split(",", str_replace('"', '', $title_line));
	...
	function submitJournalEntry() {
		...
		while($row_cnt < count($this->records)) {
			...
			$glEntry->description = BB_IMPORT_MSG;
...
//					$glEntry->ship_contact = COMPANY_CONTACT;
				...
				switch (JOURNAL_ID) {
					case 4:
						$glEntry->journal_rows[] = array(
							...
							'debit_amount' => $currencies->clean_value($order['total_cost']),
					...
					case 6:
						$glEntry->journal_rows[] = array(
							'gl_type' => BB_GL_TYPE,
							'qty' => '1',
							'description' => constant('GL_MSG_IMPORT_' . JOURNAL_ID),
							'gl_account' => $order['inv_gl_acct'],
							'taxable' => ($order['taxable']) ? $order['taxable'] : 0,
					...
					case 10:
						$glEntry->journal_rows[] = array(
							...
							'gl_account' => $order['inv_gl_acct'],
							'taxable' => ($order['taxable']) ? $order['taxable'] : 0,
							'credit_amount' => $currencies->clean_value($order['total_cost']),
							'post_date' => $order['post_date']);
						break;
					case 12:
						$glEntry->journal_rows[] = array(
							...
							'gl_account' => $order['inv_gl_acct'],
							'taxable' => ($order['taxable']) ? $order['taxable'] : 0,
							'credit_amount' => $currencies->clean_value($order['total_amount']),
							'post_date' => $order['post_date']);
						break;
				}
			...
			$glEntry->journal_rows[] = array(
				'gl_type' => 'ttl',
				'description' => constant('GENERAL_JOURNAL_' . JOURNAL_ID . '_LEDGER_HEADING'),
		...
		if (is_array($sku_list)) {
			...
			foreach ($sku_list as $sku => $details) {
				$sql = "update " . TABLE_INVENTORY . " 
					set quantity_on_hand = quantity_on_hand + " . $details['qty'] . " 
					where sku = '" . $sku . "'";
				$result = $db->Execute($sql);
				if ($result->AffectedRows() <> 1) {
					$messageStack->add(GL_BEG_BAL_ERROR_8A . $sku . GL_BEG_BAL_ERROR_8B,'error');
					$db->transRollback();
					return false;
				}
				$history_array = array(
			...
			foreach ($coa_list as $account => $amount) {
				$sql = "update " . TABLE_CHART_OF_ACCOUNTS_HISTORY . " 
					set beginning_balance = beginning_balance + " . $amount . " 
					where account_id = '" . $account . "' and period = 1";
				$result = $db->Execute($sql);
				if ($result->AffectedRows() <> 1) {
					$messageStack->add(GL_BEG_BAL_ERROR_9A . $account . GL_BEG_BAL_ERROR_8B,'error');
					$db->transRollback();
					return false;
				}
			}
