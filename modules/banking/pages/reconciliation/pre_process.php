...
// +-----------------------------------------------------------------+
//  Path: /modules/banking/pages/reconciliation/pre_process.php
//
...
switch ($action) {
  case 'save':
	...
	if (count($mains)) {
	  // closes if any cash records within the journal main are reconciled
	  $db->Execute("update " . TABLE_JOURNAL_MAIN . " m inner join " . TABLE_JOURNAL_ITEM . " i on m.id = i.ref_id
	    set m.closed = '1' 
		  where i.reconciled = '1' 
		  and i.gl_account in ('" . implode("','", $gl_accounts) . "') 
		  and m.id in (" . implode(",", $mains) . ")");
	  // re-opens if any cash records within the journal main are not reconciled
	  $db->Execute("update " . TABLE_JOURNAL_MAIN . " m inner join " . TABLE_JOURNAL_ITEM . " i on m.id = i.ref_id
	    set m.closed = '0' 
