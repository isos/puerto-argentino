...
// +-----------------------------------------------------------------+
//  Path: /modules/services/payment/modules/nova_xml.php
//
...
class nova_xml {
  ...
  function nova_xml() {
    global $order;
    $this->code = 'nova_xml';
    $this->title = MODULE_PAYMENT_NOVA_XML_TEXT_TITLE; // Payment module title in Catalog
    $this->description = MODULE_PAYMENT_NOVA_XML_TEXT_DESCRIPTION; // Descriptive Info about module in Admin
    $this->enabled = ((MODULE_PAYMENT_NOVA_XML_STATUS == 'True') ? true : false); // Whether the module is installed or not
    $this->sort_order = MODULE_PAYMENT_NOVA_XML_SORT_ORDER; // Sort Order of this payment option on the customer payment page
	$this->enable_encryption = 1; // set to field position of credit card to create hint, false to turn off encryption

    if ((int)MODULE_PAYMENT_NOVA_XML_ORDER_STATUS_ID > 0) {
      $this->order_status = MODULE_PAYMENT_NOVA_XML_ORDER_STATUS_ID;
    }
  ...
  function javascript_validation() {
    ...
    if (MODULE_PAYMENT_NOVA_XML_USE_CVV == 'True')  {
      $js .= '    var cc_cvv = document.getElementById("nova_xml_field_4").value;' . "\n";
    }
  ...
  function selection() {
	...
	$selection = array(
	   'id' => $this->code,
	   'module' => MODULE_PAYMENT_NOVA_XML_TEXT_CATALOG_TITLE,
	   'fields' => array(array('title' => MODULE_PAYMENT_NOVA_XML_TEXT_CREDIT_CARD_OWNER,
					   'field' => html_input_field('nova_xml_field_0', $order->nova_xml_field_0)),
				   array('title' => MODULE_PAYMENT_NOVA_XML_TEXT_CREDIT_CARD_NUMBER,
					   'field' => html_input_field('nova_xml_field_1', $order->nova_xml_field_1)),
				   array('title' => MODULE_PAYMENT_NOVA_XML_TEXT_CREDIT_CARD_EXPIRES,
				   ...
				   array('title' => MODULE_PAYMENT_NOVA_XML_TEXT_CVV,
  ...
  function pre_confirmation_check() {
    ...
    $result = $cc_validation->validate($_POST['nova_xml_field_1'], $_POST['nova_xml_field_2'], $_POST['nova_xml_field_3'], $_POST['nova_xml_field_4']);
    $error = '';
    switch ($result) {
      case -1:
    ...
    $this->cc_cvv2 = $_POST['nova_xml_field_4'];
  ...
  function before_process() {
    ...
    $submit_data = array(
		...
		'ssl_cvv2cvc2_indicator' => ($this->cc_cvv2) ? '1' : '9', // if cvv2 exists, present else not present
		'ssl_cvv2cvc2'           => ($this->cc_cvv2) ? $this->cc_cvv2 : '',
		...
		'ssl_phone'              => $order->bill_telephone,
...
*/
	...
	$GetPost = 'GET';	
//echo 'transmit url = '; echo htmlspecialchars($url); echo '<br /><br />';
...
//echo 'response xml = ' . htmlspecialchars($authorize) . '<br />';
  ...
  function check() {
    ...
    if (!isset($this->_check)) {
      $check_query = $db->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_NOVA_XML_STATUS'");
  ...
  function install() {
    global $db;
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable Nova Network Module', 'MODULE_PAYMENT_NOVA_XML_STATUS', 'True', 'Do you want to accept Nova Network payments ?', '6', '0', 'cfg_select_option(array(\'True\', \'False\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Merchant ID', 'MODULE_PAYMENT_NOVA_XML_MERCHANT_ID', '', 'The Nova Merchant ID used for the Nova Network service:', '6', '0', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('User ID', 'MODULE_PAYMENT_NOVA_XML_USER_ID', '', 'The Nova User ID used for the Nova Network service:', '6', '0', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('PIN', 'MODULE_PAYMENT_NOVA_XML_PIN', '', 'PIN used for the Nova Network service:', '6', '0', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Transaction Mode', 'MODULE_PAYMENT_NOVA_XML_TESTMODE', 'Test', 'Transaction mode used for processing orders', '6', '0', 'cfg_select_option(array(\'Test\', \'Production\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Authorization Type', 'MODULE_PAYMENT_NOVA_XML_AUTHORIZATION_TYPE', 'Authorize', 'Do you want submitted credit card transactions to be authorized only, or authorized and captured?', '6', '0', 'cfg_select_option(array(\'Authorize\', \'Authorize/Capture\'), ', now())");
//    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Store the Credit Card Number', 'MODULE_PAYMENT_NOVA_XML_STORE_NUMBER', 'False', 'Do you want to store the Credit Card Number. Security Note: The Credit Card Number will be stored unenecrypted.', '6', '0', 'cfg_select_option(array(\'True\', \'False\'), ', now())");
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort order of display.', 'MODULE_PAYMENT_NOVA_XML_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
  }
  /**
  ...
  function keys() {
    return array(
	  'MODULE_PAYMENT_NOVA_XML_STATUS', 
	  'MODULE_PAYMENT_NOVA_XML_MERCHANT_ID', 
	  'MODULE_PAYMENT_NOVA_XML_USER_ID', 
	  'MODULE_PAYMENT_NOVA_XML_PIN', 
	  'MODULE_PAYMENT_NOVA_XML_TESTMODE', 
	  'MODULE_PAYMENT_NOVA_XML_AUTHORIZATION_TYPE', 
//	  'MODULE_PAYMENT_NOVA_XML_STORE_NUMBER', 
	  'MODULE_PAYMENT_NOVA_XML_SORT_ORDER' 
	);
  }
}
