...
// +-----------------------------------------------------------------+
//  Path: /modules/general/pages/index/pre_process.php
//

require_once (DIR_FS_CLASSES . 'ctl_panel.php');

$page_id = $_GET['tpl'] ? $_GET['tpl'] : 'index'; // default to index unless heading is passed
$action = $_POST['action'];
switch ($action) {
	case 'save':
	...
	case 'move_down':
		...
		$sql = "select column_id, row_id from " . TABLE_USERS_PROFILES . " 
			where user_id = " . $_SESSION['admin_id'] . " and page_id = '" . $page_id . "' and module_id = '" . $module_id . "'";
		...
		$sql = "select max(row_id) as max_row from " . TABLE_USERS_PROFILES . " 
			where user_id = " . $_SESSION['admin_id'] . " and page_id = '" . $page_id . "' and column_id = '" . $current_column . "'";
		$result         = $db->Execute($sql);
	...
	case 'move_right':
		...
		if (($new_col >= 1 && $action == 'move_left') || ($new_col <= MAX_CP_COLUMNS && $action == 'move_right')) {
			$sql = "select max(row_id) as max_row from " . TABLE_USERS_PROFILES . " 
				where user_id = " . $_SESSION['admin_id'] . " and page_id = '" . $page_id . "' and column_id = '" . $new_col . "'";
			$result = $db->Execute($sql);
...
// prepare to display templatez
$cp_boxes = $db->Execute("select * from " . TABLE_USERS_PROFILES . " 
	where user_id = " . $_SESSION['admin_id'] . " and page_id = '" . $page_id . "' order by column_id, row_id");
