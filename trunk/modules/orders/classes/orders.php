...
// +-----------------------------------------------------------------+
//  Path: /modules/orders/classes/orders.php
//
...
// class constructor
	...
	function post_ordr($action) {
		...
		if ($this->recur_id > 0) { // if new record, will contain count, if edit will contain recur_id
			...
			if ($this->id) { // it's an edit, fetch list of affected records to update if roll is enabled
				...
				for ($i = 0; $i < $this->recur_id; $i++) {
					...
					$this->remove_cogs_rows();
					for ($j = 0; $j < count($this->journal_rows); $j++) $this->journal_rows[$j]['id'] = '';
					$this->post_date     = gen_specific_date($this->post_date, $day_offset, $month_offset, $year_offset);
		...
		switch ($this->journal_id) {
			...
			case 21: // POP Journal (Direct Inventory)
			default:
				break;
		}
...
//echo 'committed transaction - bailing!'; exit();
		// ***************************** END TRANSACTION *******************************
		$messageStack->add_session(constant('ORD_TEXT_' . $this->journal_id . '_WINDOW_TITLE') . ' Ref # ' . $this->purchase_invoice_id . ' registrada satisfactoriamente', 'success');
		return true;
	}
	...
	function add_total_journal_row($debit_credit) {	// put total value into ledger row array
		switch ($this->journal_id) { // code for total gl_entry amount
			case 19:
			case 21: $gl_type = 'pmt'; break; // POS and POP are a direct payments
			default: $gl_type = 'ttl'; break;
		}
		...
		if ($debit_credit == 'debit' || $debit_credit == 'credit') {
			$this->journal_rows[] = array( // record for accounts receivable
				...
				'description'             => constant('GENERAL_JOURNAL_' . $this->journal_id . '_LEDGER_HEADING'),
	  ...
	  if ($debit_credit == 'debit' || $debit_credit == 'credit') {
		if ($this->discount <> 0) {
		  $this->journal_rows[] = array(
			...
			'description'             => constant('GENERAL_JOURNAL_' . $this->journal_id . '_LEDGER_DISCOUNT'),
	...
	function add_freight_journal_row($debit_credit) {	// put freight into journal row array
	  if ($debit_credit == 'debit' || $debit_credit == 'credit') {
		switch ($this->journal_id) {
		  ...
		  case  7:
		  case 21:
			$freight_tax_id = AP_ADD_SALES_TAX_TO_SHIPPING;
			break;
		  case  9:
		  case 10: 
		  case 12:
		  case 13:
		  case 19:
			$freight_tax_id = AR_ADD_SALES_TAX_TO_SHIPPING;
			break;
		  default:
		}
		if ($this->freight) { // calculate freight charges
			$this->journal_rows[] = array(
				...
				'description'             => constant('GENERAL_JOURNAL_' . $this->journal_id . '_LEDGER_FREIGHT'),
