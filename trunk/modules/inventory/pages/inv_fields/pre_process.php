...
// +-----------------------------------------------------------------+
//  Path: /modules/inventory/pages/inv_fields/pre_process.php
//

/**************   Check user security   *****************************/
$security_level = $_SESSION['admin_security'][SECURITY_ID_INV_FIELDS];
...
$custom_path = DIR_FS_MY_FILES . 'custom/inventory/inv_fields/extra_actions.php';
...
switch ($action) {
  ...
  case 'update':
	...
	$inv_field_id = (int)db_prepare_input($_POST['rowSeq']);
...
// result error num and text and show the error in messgeStack NOT crash.
	...
	$sql = "select inv_field_id from " . TABLE_INVENTORY_FIELDS . " where field_name = '" . $field_name . "'";
...
//TBD - evaluate the format entered to see if it makes sense
	...
	if (!$error) {
		...
		if ($action == 'save') {
			$inv_field_id = db_insert_id();
			$sql = "alter table " . TABLE_INVENTORY . " 
				add column " . $field_name . " " . $values['entry_type'] . (isset($values['entry_params']) ? $values['entry_params'] : '');
			$db->Execute($sql);
			db_perform(TABLE_INVENTORY_FIELDS, $sql_data_array, 'insert');
			gen_add_audit_log(INV_LOG_FIELDS . TEXT_NEW, $category_id . ' - ' . $field_name);
		} else {
			if (isset($values['entry_type'])) {
				$sql = "alter table " . TABLE_INVENTORY . " change " . $field_name . " " . $field_name . " 
					" . $values['entry_type'] . (isset($values['entry_params']) ? $values['entry_params'] : '');
				$result = $db->Execute($sql);
			}
			db_perform(TABLE_INVENTORY_FIELDS, $sql_data_array, 'update', "inv_field_id = " . $inv_field_id);
			gen_add_audit_log(INV_LOG_FIELDS . TEXT_UPDATE, $category_id . ' - ' . $field_name);
		}
		gen_redirect(html_href_link(FILENAME_DEFAULT, gen_get_all_get_params(array('action')), 'SSL'));
	} else {
  ...
  case 'delete':
	...
	$temp = $db->Execute("select field_name, category_id from " . TABLE_INVENTORY_FIELDS . " where inv_field_id = " . $inv_field_id);
	...
	if ($category_id <> '0') { // don't allow deletion of system fields
		$db->Execute("delete from " . TABLE_INVENTORY_FIELDS . " where inv_field_id = " . $inv_field_id);
		$db->Execute("alter table " . TABLE_INVENTORY . " drop column " . $field_name);
		gen_add_audit_log(INV_LOG_FIELDS . TEXT_DELETE, $category_id . ' - ' . $field_name);
	} else {
  ...
  case 'edit':
	...
	$inventory = $db->Execute("select inv_field_id, entry_type, field_name, description, category_id, params 
		from " . TABLE_INVENTORY_FIELDS . " 
		where inv_field_id = " . (int)$_POST['rowSeq']);
...
switch ($action) {
  ...
  default:
	...
	$field_list = array('inv_field_id', 'field_name', 'entry_type', 'description', 'category_id');

	// hook to add new fields to the query return results
	if (is_array($extra_query_list_fields) > 0) $field_list = array_merge($field_list, $extra_query_list_fields);

    $query_raw = "select " . implode(', ', $field_list)  . " from " . TABLE_INVENTORY_FIELDS . $search . " order by $disp_order";
