...
$fID = db_prepare_input($_GET['fID']); // specifies the return function ID for the handler
if (!$fID) $fID = 'invDetails'; // set default if return function not passed
...
// some error checking
if (!$sku && !$UPC && !$iID) {
  echo createXmlHeader($fID) . xmlEntry('error', AJAX_INV_NO_INFO) . createXmlFooter();
  die;
}
...
if ($UPC && $inventory->RecordCount() <> 1) { // for UPC codes submitted only, send an error
  echo createXmlHeader($fID) . xmlEntry('error', ORD_JS_SKU_NOT_UNIQUE) . createXmlFooter();
  die;
} elseif ($inventory->RecordCount() <> 1) { // for everyhing else if not exactly 1 match just die and don't return anything
  die;
}
	...
	if ($qty > $inventory->fields['branch_qty_in_stock'] && strpos(COG_ITEM_TYPES, $inventory->fields['inventory_type']) !== false) {
	  ...
	  if (is_array($sku_history['open_po'])) {
	    ...
	    foreach ($sku_history['open_po'] as $value) {
		  $store = $value['store_id'] ? gen_get_account_name($value['store_id']) : COMPANY_NAME; // get name from id
