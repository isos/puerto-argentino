...
// +-----------------------------------------------------------------+
//  Path: /modules/reportwriter/classes/entry_builder.php
//

if (file_exists(DIR_FS_WORKING . 'language/' . $_SESSION['language'] . '/classes/entry_builder.php')) {
  require(DIR_FS_WORKING . 'language/' . $_SESSION['language'] . '/classes/entry_builder.php');
} else {
  require(DIR_FS_WORKING . 'language/en_us/classes/entry_builder.php');
}
...
//echo 'line items = '; print_r($this->line_items); echo '<br />';
	...
	function load_item_details($id) {
		...
		while (!$result->EOF) {
		  ...
		  if ($result->fields['gl_type'] == 'sos') {
			$this->line_items[$index]['invoice_price']       = $result->fields['credit_amount'];
			$this->line_items[$index]['invoice_full_price']  = $result->fields['full_price'];
			$this->line_items[$index]['invoice_unit_price']  = ($result->fields['qty']) ? ($result->fields['credit_amount'] / $result->fields['qty']) : 0;
			$this->line_items[$index]['invoice_discount']    = ($result->fields['full_price'] == 0) ? 0 : ($result->fields['full_price'] - ($result->fields['credit_amount'] / $result->fields['qty'])) / $result->fields['full_price'];
			...
			$this->invoice_subtotal += $result->fields['credit_amount'];
		  ...
		  if ($result->fields['gl_type'] == 'tax') {
		    $tax_list[] = $result->fields['description'] . ' - ' . $currencies->format_full($result->fields['credit_amount']);
		  }
		  if ($result->fields['gl_type'] == 'dsc') $this->discount       = $result->fields['debit_amount'];
		  $result->MoveNext();
		}
	...
	function load_shipment_details($id) {
		...
		if (file_exists(DIR_FS_MODULES  . 'services/shipping/modules/' . $carrier . '.php')) {
			include_once(DIR_FS_MODULES . 'services/shipping/modules/' . $carrier . '.php');
			$ship_info = new $carrier();
			$this->ship_carrier = ($ship_info->title_short) ? $ship_info->title_short : $ship_info->title;
			$this->ship_service = constant($carrier . '_' . $shipping_info[1]);
		} elseif (file_exists(DIR_FS_MY_FILES  . 'custom/services/shipping/modules/' . $carrier . '.php')) {
			include_once(DIR_FS_MY_FILES . 'custom/services/shipping/modules/' . $carrier . '.php');
			$ship_info = new $carrier();
			$this->ship_carrier = ($ship_info->title_short) ? $ship_info->title_short : $ship_info->title;
			$this->ship_service = constant($carrier . '_' . $shipping_info[1]);
		}
...
//		$output[] = array('id' => 'ship_telephone2',     'text' => RW_EB_SHIP_TELE2);
//		$output[] = array('id' => 'ship_telephone3',     'text' => RW_EB_SHIP_FAX);
//		$output[] = array('id' => 'ship_telephone4',     'text' => RW_EB_SHIP_TELE4);
		$output[] = array('id' => 'ship_email',          'text' => RW_EB_SHIP_EMAIL);
//		$output[] = array('id' => 'ship_website',        'text' => RW_EB_SHIP_WEBSITE);
