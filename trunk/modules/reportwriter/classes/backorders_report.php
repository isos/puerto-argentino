...
// +-----------------------------------------------------------------+
//  Path: /modules/reportwriter/classes/backorders_report.php
//
...
if (file_exists(DIR_FS_WORKING . 'language/' . $_SESSION['language'] . '/classes/backorders_report.php')) {
  require(DIR_FS_WORKING . 'language/' . $_SESSION['language'] . '/classes/backorders_report.php');
} else {
  require(DIR_FS_WORKING . 'language/en_us/classes/backorders_report.php');
}
...
// echo 'output array = '; print_r($OutputArray); echo '<br />'; exit();
  ...
  function build_selection_dropdown() {
	...
	$output = array();
	$output[] = array('id' => '[table1].id',                  'text' => RW_BO_RECORD_ID);
	$output[] = array('id' => '[table1].period',              'text' => TEXT_PERIOD);
	$output[] = array('id' => '[table1].post_date',           'text' => TEXT_POST_DATE);
	$output[] = array('id' => '[table1].store_id',            'text' => RW_BO_STORE_ID);
	$output[] = array('id' => '[table1].freight',             'text' => RW_BO_FRT_TOTAL);
	$output[] = array('id' => '[table1].ship_carrier',        'text' => RW_BO_FRT_CARRIER);
	$output[] = array('id' => '[table1].ship_service',        'text' => RW_BO_FRT_SERVICE);
	$output[] = array('id' => '[table1].sales_tax',           'text' => RW_BO_SALES_TAX);
//	$output[] = array('id' => '[table1].tax_auths',           'text' => RW_BO_TAX_AUTH);
	$output[] = array('id' => '[table1].total_amount',        'text' => RW_BO_INV_TOTAL);
	$output[] = array('id' => '[table1].currencies_code',     'text' => RW_BO_CUR_CODE);
	$output[] = array('id' => '[table1].currencies_value',    'text' => RW_BO_CUR_EXC_RATE);
	$output[] = array('id' => '[table1].purchase_invoice_id', 'text' => RW_BO_INV_NUM);
	$output[] = array('id' => '[table1].purch_order_id',      'text' => RW_BO_PO_NUM);
	$output[] = array('id' => '[table1].rep_id',              'text' => RW_BO_SALES_REP);
	$output[] = array('id' => '[table1].gl_acct_id',          'text' => RW_BO_AR_ACCT);
	$output[] = array('id' => '[table1].bill_acct_id',        'text' => RW_BO_BILL_ACCT_ID);
	$output[] = array('id' => '[table1].bill_address_id',     'text' => RW_BO_BILL_ADD_ID);
	$output[] = array('id' => '[table1].bill_primary_name',   'text' => RW_BO_BILL_PRIMARY_NAME);
	$output[] = array('id' => '[table1].bill_contact',        'text' => RW_BO_BILL_CONTACT);
	$output[] = array('id' => '[table1].bill_address1',       'text' => RW_BO_BILL_ADDRESS1);
	$output[] = array('id' => '[table1].bill_address2',       'text' => RW_BO_BILL_ADDRESS2);
	$output[] = array('id' => '[table1].bill_city_town',      'text' => RW_BO_BILL_CITY);
	$output[] = array('id' => '[table1].bill_state_province', 'text' => RW_BO_BILL_STATE);
	$output[] = array('id' => '[table1].bill_postal_code',    'text' => RW_BO_BILL_ZIP);
	$output[] = array('id' => '[table1].bill_country_code',   'text' => RW_BO_BILL_COUNTRY);
	$output[] = array('id' => '[table1].bill_telephone1',     'text' => RW_BO_BILL_TELE1);
	
	$output[] = array('id' => '[table2].qty',                 'text' => RW_BO_QTY_ORDERED);
	$output[] = array('id' => '[table2].sku',                 'text' => TEXT_SKU);
	$output[] = array('id' => '[table2].description',         'text' => TEXT_DESCRIPTION);
	$output[] = array('id' => '[table3].quantity_on_hand',    'text' => RW_QTY_IN_STOCK);
	$output[] = array('id' => '[table2].qty_backorder',       'text' => RW_BO_QTY_BACKORDER);
	return $output;
  }
