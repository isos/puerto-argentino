...
// +-----------------------------------------------------------------+
//  Path: /modules/services/shipping/label_mgr/fedex/js_include.php
//
...
var dateShipped = new ctlSpiffyCalendarBox("dateShipped", "label_mgr", "terminal_date","btnDate2", "<?php echo date('m/d/Y', time()); ?>", scBTNMODE_CALBTN);

function init() {
	<?php if (!$error && ($action == 'label' || $action == 'delete')) {
...
function addRow() {
	...
	cell[0] += buildIcon(image_path+'16x16/emblems/emblem-unreadable.png', image_delete_text, 'style="cursor:pointer" onclick="if (confirm(\''+image_delete_msg+'\')) removeRow('+rowCnt+');"') + '</td>';
