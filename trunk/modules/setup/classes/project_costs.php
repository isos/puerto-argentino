...
// +-----------------------------------------------------------------+
//  Path: /modules/setup/classes/project_costs.php
//

class project_costs {

  function project_costs() {
  	$this->security_id   = $_SESSION['admin_security'][SECURITY_ID_PROJECT_COSTS];
	$this->db_table      = TABLE_PROJECTS_COSTS;
	$this->title         = SETUP_TITLE_PROJECTS_COSTS;
...
//	$toolbar->icon_list['update']['text'] = 'Text override here';
  ...
  function btn_save($id = '') {
    ...
    if ($id) {
	  db_perform($this->db_table, $sql_data_array, 'update', "cost_id = '" . (int)$id . "'");
  ...
  function btn_delete() {
    ...
    $cost_id = db_prepare_input($_POST['rowSeq']);
/*
...
*/
	// OK to delete
	$result = $db->Execute("select description_short from " . $this->db_table . " where cost_id = " . (int)$cost_id);
	$db->Execute("delete from " . $this->db_table . " where cost_id = " . (int)$cost_id);
  ...
  function build_main_html() {
    ...
    while (!$result->EOF) {
      ...
      $output .= '    <td class="dataTableContent" onclick="loadPopUp(\'edit\', ' . $result->fields['cost_id'] . ')">' . htmlspecialchars($result->fields['description_short']) . '</td>' . chr(10);
      $output .= '    <td class="dataTableContent" onclick="loadPopUp(\'edit\', ' . $result->fields['cost_id'] . ')">' . $project_cost_types[$result->fields['cost_type']] . '</td>' . chr(10);
      $output .= '    <td class="dataTableContent" onclick="loadPopUp(\'edit\', ' . $result->fields['cost_id'] . ')">' . ($result->fields['inactive'] ? TEXT_YES : '') . '</td>' . chr(10);
      $output .= '    <td class="dataTableContent" align="right">' . chr(10);
	  if ($this->security_id > 1) $output .= html_icon('actions/edit-find-replace.png', TEXT_EDIT,   'small', 'onclick="loadPopUp(\'edit\', ' . $result->fields['cost_id'] . ')"') . chr(10);
	  if ($this->security_id > 3) $output .= html_icon('emblems/emblem-unreadable.png', TEXT_DELETE, 'small', 'onclick="if (confirm(\'' . SETUP_PROJECT_COSTS_DELETE_INTRO . '\')) submitSeq(' . $result->fields['cost_id'] . ', \'delete\')"') . chr(10);
      $output .= '    </td>' . chr(10);
      $output .= '  </tr>' . chr(10);
      $result->MoveNext();
    }
    ...
    $output .= '<div class="page_count">' . $page_split->display_count($query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $_GET['page'], SETUP_DISPLAY_NUMBER_OF_PROJECT_COSTS) . '</div>' . chr(10);
	return $output;
  }
  ...
  function build_form_html($action, $id = '') {
	...
	$output .= '    <td colspan="2">' . ($action=='new' ? SETUP_PROJECT_COSTS_INSERT_INTRO : SETUP_PROJECT_COSTS_EDIT_INTRO) . '</td>' . chr(10);
    $output .= '  </tr>' . chr(10);
	$output .= '  <tr class="dataTableRow">' . chr(10);
