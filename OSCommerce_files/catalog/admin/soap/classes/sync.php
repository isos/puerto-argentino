...
//echo 'parsed string at shopping cart = '; print_r($this->arrOutput); echo '<br>';
  ...
  function formatArray() { // specific to XML spec for a product sync
	...
	for ($i = 0; $i < count($itemArray); $i++) {
	  ...
	  $this->sku_array[] = urldecode($this->getNodeData(array('SKU'), $tempArray));
	}
	return true;
  }
...
// It also needs to check for errors, i.e. missing information, bad data, etc. 
  function syncProducts() {
	...
	$missing_skus = array();
	$result = tep_db_query("select phreebooks_sku from " . TABLE_PRODUCTS);
	while($fields = tep_db_fetch_array($result)) {
	  if (!in_array($fields['phreebooks_sku'], $this->sku_array)) {
		$missing_skus[] = $fields['phreebooks_sku'];
	  }
	}
