...
// It also needs to check for errors, i.e. missing information, bad data, etc. 
  function orderConfirm() {
    ...
    foreach ($this->order_array as $value) { 
	  $result = tep_db_query("select orders_status from " . TABLE_ORDERS . " where orders_id = '" . (int)$value['id'] . "'");
	  $fields = tep_db_fetch_array($result);
	  if (tep_db_num_rows($result) == 0 || $fields['orders_status'] == $value['status']) continue; // skip this order, not a osc order
